...
|---|---|---|---|
| `rte_lcore_var` (24.11) | **N/A** — 0 candidate sites | `adopt-rte_lcore_var.md` | Architectural mismatch: our Stage 1 design is single-Engine-per-lcore (Cell/RefCell), not the per-lcore-array pattern this API replaces. The 5 `repr(C, align(64))` decorations in `counters.rs` are single-instance struct decorations for false-sharing isolation, not the per-lcore-slab pattern. |
| `rte_ptr_compress` (24.07) | **deferred-to-e2e** | `adopt-rte_ptr_compress.md` | 1 candidate site exists at `engine.rs:2077` (the `[*mut rte_mbuf; 32]` RX burst array), but `EngineNoEalHarness` doesn't call `shim_rte_eth_rx_burst`, and send is stubbed. Bench-micro has no measurement reach. Re-evaluate when bench-e2e runs on real ENA. |
| `rte_bit_atomic_*` (24.11) | **N/A** — 0 candidate sites | `adopt-rte_bit_atomic.md` | Codebase scan found 0 instances of `fetch_or` / `fetch_and` / `fetch_xor` / `fetch_nand` anywhere. Our atomic surface is exclusively `fetch_add(_, Relaxed)` and `load(Relaxed)` on counters. The bit-mask atomic pattern this API optimizes simply doesn't exist in our code. |
