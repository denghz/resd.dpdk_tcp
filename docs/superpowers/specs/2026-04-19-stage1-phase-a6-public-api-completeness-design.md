...
- **Public timer API** — `dpdk_net_timer_add(deadline_ns, user_data, *timer_id_out) → i32`, `dpdk_net_timer_cancel(timer_id) → i32`, new event `DPDK_NET_EVT_TIMER {timer_id, user_data}` layered on A5's internal hashed timing wheel (parent spec §7.4). The wheel already reserves `TimerKind::ApiPublic`; A6 populates the fire path and the public-API plumbing.
- **Send-buffer backpressure event** — `DPDK_NET_EVT_WRITABLE` emitted once per prior-refusal cycle when in-flight drains to ≤ `send_buffer_bytes / 2`.
- **TX-batched `dpdk_net_flush`** — engine-scope TX ring holds outbound data-segment mbufs; `dpdk_net_flush` drains the ring via exactly one `rte_eth_tx_burst`. Control frames (ACK / SYN / FIN / RST) remain emitted inline and do not participate in the ring. This is the (c) option chosen during brainstorm: latency-optimal for the data path, no ACK latency inversion. Parent spec §4.2 clarified in §10.2 of this doc to reflect the split.
- **`dpdk_net_close(flags)` with `FORCE_TW_SKIP`** — honored only when the closing connection negotiated timestamps (RFC 6191 / RFC 7323 §5 prerequisite for PAWS to protect 4-tuple reuse on the client side); otherwise the flag is ignored and a `DPDK_NET_EVT_ERROR{err=-EPERM}` event is emitted (parent spec §9.3 lists this as the documented "EPERM_TW_REQUIRED" condition — we reuse `-EPERM` as the errno since no dedicated `EPERM_TW_REQUIRED` exists in `libc`; the contextual meaning is documented in the cbindgen header's `dpdk_net_close` doc-comment).
...
- **Mempool exhaustion → `DPDK_NET_EVT_ERROR{err=-ENOMEM}`** — per-occurrence for internal retransmit ENOMEM; edge-triggered per poll iteration for RX mempool drops (one event per poll iteration where `eth.rx_drop_nomem` advanced). `dpdk_net_send` already returns `-ENOMEM` synchronously on TX-data-mempool exhaustion and stays unchanged.
- **`preset=rfc_compliance`** — new `dpdk_net_engine_config_t::preset` value `1` forces `tcp_nagle=true`, `tcp_delayed_ack=true`, `cc_mode=1` (Reno), `tcp_min_rto_us=200_000`, `tcp_initial_rto_us=1_000_000`. Other A5+ fields pass through. Applied post-zero-sentinel-substitution inside `dpdk_net_engine_create`. `preset >= 2` rejected with null-return.
...
- **Per-connection RTT histogram (absorbed from A5.6)** — 16 × `u32` log-spaced buckets on `TcpConn`, exactly 64 B / one cacheline, runtime-configurable edges via `dpdk_net_engine_config_t::rtt_histogram_bucket_edges_us[15]`, wraparound via `wrapping_add` with app-side `wrapping_sub` on delta. New `dpdk_net_tcp_rtt_histogram_t { uint32_t bucket[16] }` POD. New extern "C" `dpdk_net_conn_rtt_histogram(engine, conn, *out) → i32`.
- **Knob-coverage audit entries** for the three new behavioral knobs: `preset=rfc_compliance`, `dpdk_net_close` flag `FORCE_TW_SKIP`, `rtt_histogram_bucket_edges_us`. Sibling `tests/per-conn-histogram-coverage.rs` file for the histogram's per-conn coverage (engine-wide audit doesn't reach per-conn state).
...
- Benchmarks (A10).
- Per-sample `DPDK_NET_EVT_RTT_SAMPLE` events or raw-samples ring — deferred indefinitely; histogram covers the stated observability need.
- Engine-wide RTT histogram summary (sum across conns) — deferred per A5.6 §12; apps sum across `dpdk_net_conn_rtt_histogram` snapshots on demand.
...
- A-HW offload territory — offload bits, port config, feature flags. A6 and A-HW touch `engine.rs` / `api.rs` / `include/dpdk_net.h` but in disjoint regions (see parent plan's coordination note); rebase cadence covers overlap.
...
| Module | Change |
|---|---|
| `api.rs` | New POD `dpdk_net_tcp_rtt_histogram_t { uint32_t bucket[16] }` (64 B, one cacheline, `#[repr(C)]`). New config field `dpdk_net_engine_config_t::rtt_histogram_bucket_edges_us: [u32; 15]` (appended). `dpdk_net_timer_id_t` (existing `u64` alias) contract documented: packed `(slot << 32) \| generation`. No change to existing public types' layout beyond the appended `rtt_histogram_bucket_edges_us` field. |
| `lib.rs` | Implementations: `dpdk_net_timer_add` (null checks → `engine.public_timer_add` → pack `TimerId` to `u64` → `*timer_id_out`); `dpdk_net_timer_cancel` (null check → `engine.public_timer_cancel` → `0` or `-ENOENT`); `dpdk_net_conn_rtt_histogram` (null checks → flow-table lookup → `std::ptr::write` the 16 bucket copies into the caller's out struct); `dpdk_net_flush` (already a no-op shell — replace body with `engine.drain_tx_pending_data()`); `dpdk_net_close` (extend to read the `flags` param, call `engine.close_conn_with_flags(handle, flags)`, map errors). `dpdk_net_engine_create` adds the preset application + histogram-edges validation + plumbs the edges into `EngineConfig`. `build_event_from_internal` handles the two new variants (`ApiTimer`, `Writable`). |
| `include/dpdk_net.h` (cbindgen-regenerated) | New struct `dpdk_net_tcp_rtt_histogram_t`. New extern "C" functions `dpdk_net_timer_add`, `dpdk_net_timer_cancel`, `dpdk_net_conn_rtt_histogram`. New config field `rtt_histogram_bucket_edges_us`. Updated doc-comments on `dpdk_net_flush` (explicit data-only drain semantics), `dpdk_net_close` (flag + error-code contract). |
...
A `repr(C, align(64))` single-cacheline newtype guarantees the 16 × `u32` buckets live on exactly one cacheline regardless of the surrounding `TcpConn` layout. The snapshot getter returns a `[u32; 16]` memcpy (no pointer aliasing into the internal field — the caller's `dpdk_net_tcp_rtt_histogram_t` owns its own 64 B).
...
**Add.** Caller passes `deadline_ns` in the engine monotonic-clock domain (`dpdk_net_now_ns`) and an opaque `user_data: u64`. The engine rounds `deadline_ns` up to the next 10 µs wheel tick boundary (the wheel's native resolution, parent spec §7.4); deadlines already in the past fire on the next `poll_once`. Internally:
...
The `TimerId` (slot:u32 + generation:u32) is packed into a `u64` at the ABI layer: `(slot as u64) << 32 | (generation as u64)`. Unpacking is the inverse. The packing is documented in the cbindgen header's `dpdk_net_timer_add` doc-comment so apps treat `dpdk_net_timer_id_t` as opaque but know the upper 32 bits change on slot reuse.
...
Capacity is `EngineConfig::tx_ring_size` (default 1024). At `poll_once`'s end (after RX processing + timer-wheel advance + TIME_WAIT reap), `drain_tx_pending_data` is called. `dpdk_net_flush` calls the same helper. Drain:
...
fn drain_tx_pending_data(&self) {
    ...
    let sent = unsafe { sys::shim_rte_eth_tx_burst(
        self.cfg.port_id,
...
### 3.3 `DPDK_NET_EVT_WRITABLE` hysteresis
...
Level-triggered, single-edge-per-refusal-cycle. Subsequent refused sends re-arm the cycle. The threshold is a fixed `send_buffer_bytes / 2` (spec §4.2 literal). `ABI-layer translation produces `DPDK_NET_EVT_WRITABLE` with no payload (`_pad: [0u8; 16]`).

### 3.4 `dpdk_net_close(FORCE_TW_SKIP)`

At `dpdk_net_close` entry:

```rust
pub fn close_conn_with_flags(&self, handle: ConnHandle, flags: u32) -> Result<(), Error> {
    let force_tw_skip = (flags & DPDK_NET_CLOSE_FORCE_TW_SKIP) != 0;
...
`dpdk_net_engine_create` already does zero-sentinel substitution for `max_connections`, `recv_buffer_bytes`, `send_buffer_bytes`, `tcp_mss`, `tcp_min_rto_us`, `tcp_initial_rto_us`, `tcp_max_rto_us`, `tcp_max_retrans_count`, `tcp_msl_ms`. A6 adds a post-substitution preset pass:
...
### 3.6 `DPDK_NET_EVT_ERROR{err=-ENOMEM}` emission policy
...
for (handle in live_conns) {
    dpdk_net_tcp_rtt_histogram_t h;
    dpdk_net_conn_rtt_histogram(engine, handle, &h);
    for (int i = 0; i < 16; ++i) agg[i] += h.bucket[i];
}
```

O(conns) per aggregation. At Stage 1 scale (≤100 conns per roadmap §1) this is 6.4 KB of reads and a 1600-op loop — sub-µs. Adding a dedicated `dpdk_net_engine_rtt_histogram` would require walking the flow table on each call (same cost as the above loop) plus the summing, so the user-space-does-it path is strictly cheaper for the engine and avoids adding machinery for a use case that doesn't have a concrete consumer yet.
...
None extend `dpdk_net_counters_t`'s alignment / padding in a way that risks layout change — all land in the existing `tcp` cacheline block's `_pad` slots (the current `dpdk_net_tcp_counters_t` ends with `_pad: [u64; 1]` per the A5.5-era layout; A6 consumes the pad slot and adjusts as needed, with compile-time size/align asserts pinning the C-ABI invariant).

No new hot-path counters. No new feature gates.

**Observability-only events (not counters):** `ApiTimer`, `Writable` on `InternalEvent`; `DPDK_NET_EVT_TIMER`, `DPDK_NET_EVT_WRITABLE` at the ABI boundary. The TIMER event was already declared in the C header (layout reserved); A6 just populates it. WRITABLE was already declared as an enum variant; A6 adds the emit sites.
 ...
 * clock domain (dpdk_net_now_ns). Rounded up to the next 10 µs wheel tick;
 * past deadlines fire on the next poll. On fire, emits DPDK_NET_EVT_TIMER
...
int32_t dpdk_net_conn_rtt_histogram(
    dpdk_net_engine *engine,
...
|---|---|---|
| `dpdk_net_flush` | No-op (A1 stub; A3 kept stubbed) | Drains the pending data-segment TX ring via one `rte_eth_tx_burst`. No-op when ring empty. Idempotent. |
| `dpdk_net_close` | `flags` ignored | Honors `DPDK_NET_CLOSE_FORCE_TW_SKIP` per §3.4. `flags` still silently discarded for bits other than the defined `FORCE_TW_SKIP`; undefined bits reserved for future extension. |
| `dpdk_net_engine_create` | `cfg.preset` ignored | Applies `preset=1` per §3.5; rejects `preset >= 2` with null-return. Validates `cfg.rtt_histogram_bucket_edges_us` monotonicity. |
...
**Close flag:**
- `dpdk_net_close(handle, DPDK_NET_CLOSE_FORCE_TW_SKIP)` on an ESTABLISHED conn with `ts_enabled=false` emits `Error{err=-EPERM}`; `force_tw_skip` unset.
...
Land under `crates/dpdk-net-core/tests/tcp_a6_public_api_tap.rs`:

1. **Timer add/fire/drain:** `timer_add(now+5ms, user_data=0xABCD1234_5678_BEEF)` → poll loop → `DPDK_NET_EVT_TIMER` delivered with matching `timer_id` + `user_data`; `enqueued_ts_ns` within ±1 tick (10 µs) of fire instant.
...
4. **Flush batching:** send 10 MSS-sized segments; observe `tx_pending_data` grows to 10 on the send side; `dpdk_net_flush` drains in one `tx_burst`; `tcp.tx_flush_bursts == 1`, `tcp.tx_flush_batched_pkts == 10`; `tx_pending_data` empty after.
...
6. **WRITABLE hysteresis:** fill send buffer via `send_bytes` returning accepted < len; drive ACKs until in_flight ≤ send_buffer_bytes/2; exactly one `DPDK_NET_EVT_WRITABLE` event; subsequent partial-refusal produces a fresh WRITABLE cycle.
...
13. **Histogram distribution shape:** drive RTTs across 5 known buckets; `dpdk_net_conn_rtt_histogram` returns the expected counts; delta between two snapshots over a controlled interval matches.
14. **Histogram unknown handle:** `dpdk_net_conn_rtt_histogram(engine, 0xdead_beef, &out)` → `-ENOENT`; `out` unchanged.
15. **Histogram null out:** `dpdk_net_conn_rtt_histogram(engine, conn, NULL)` → `-EINVAL`; no crash.
...
### 7.3 Knob-coverage additions (`crates/dpdk-net-core/tests/knob-coverage.rs`)

Three new scenario tests:

- **`knob_preset_rfc_compliance_forces_rfc_defaults`** — construct `dpdk_net_engine_config_t { preset: 1, tcp_nagle: false, tcp_delayed_ack: false, cc_mode: 0, tcp_min_rto_us: 5_000, tcp_initial_rto_us: 5_000, .. }`; post-`dpdk_net_engine_create`, assert the five §3.5 fields were overridden.
...
**Engine machinery:**
5. `Engine::tx_pending_data` ring + `drain_tx_pending_data` + `poll_once` end-of-iter drain + `dpdk_net_flush` wiring.
...
9. `preset=1` application in `dpdk_net_engine_create` + `preset>=2` rejection + layer-A test.
...
**Public API surface:**
17. `dpdk_net_timer_add` + `dpdk_net_timer_cancel` extern "C" functions + `api.rs` type additions + cbindgen header regen.
18. `dpdk_net_conn_rtt_histogram` extern "C" function + POD struct + cbindgen header regen.
19. `dpdk_net_close` flag-honoring body (replace the `_flags` stub); cbindgen doc-comment on flag + error-code contract.
20. `dpdk_net_engine_config_t::rtt_histogram_bucket_edges_us` field + `dpdk_net_engine_create` plumb into `EngineConfig`.
...
- Add `dpdk_net_timer_add` / `dpdk_net_timer_cancel` under the "Timers & clock" paragraph (already present in the Stage 1 API listing at parent §4 — A6 finalizes the wording around `-EALREADY` → `-ENOENT` collapse).
- Add `dpdk_net_conn_rtt_histogram` under the "Introspection (A5.5)" paragraph alongside `dpdk_net_conn_stats`.
- Add `dpdk_net_tcp_rtt_histogram_t` POD definition.
- Add `rtt_histogram_bucket_edges_us[15]` to the `dpdk_net_engine_config_t` listing.

### 10.2 §4.2 contract wording updates

- `dpdk_net_flush` clarification: "drains the pending data-segment TX batch via exactly one `rte_eth_tx_burst`. Control frames (ACK, SYN, FIN, RST) are emitted inline at their emit site and do not participate in the flush batch."
- `dpdk_net_timer_cancel` wording: `-EALREADY` collapsed into `-ENOENT`; "callers must always drain any queued TIMER events regardless of this return."
- `dpdk_net_close` flag + error-code paragraph: "When `DPDK_NET_CLOSE_FORCE_TW_SKIP` is set but the connection did not negotiate timestamps, the flag is silently dropped, a `DPDK_NET_EVT_ERROR{err=-EPERM}` event is emitted for visibility, and the normal FIN + 2×MSL TIME_WAIT sequence proceeds."
...
- Note that per-conn RTT histogram is NOT in `dpdk_net_counters_t` — read via `dpdk_net_conn_rtt_histogram`.
