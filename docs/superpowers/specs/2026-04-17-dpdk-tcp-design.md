...
typedef struct {
    ...
    uint32_t rtt_histogram_bucket_edges_us[15]; /* A6: 15 strictly monotonically
...
/* POSIX shutdown(2) subset — full-close only (Stage 1).
 *   how = DPDK_NET_SHUT_RDWR: dispatches to dpdk_net_close(engine, conn, 0).
 *   how = DPDK_NET_SHUT_RD or DPDK_NET_SHUT_WR: returns -EOPNOTSUPP
 *     (half-close not implemented; see §6.4 AD-A8.5-shutdown-no-half-close).
 *   any other how: returns -EINVAL.
 * Returns 0 on success; negative errno on failure. Use dpdk_net_close
 * directly when DPDK_NET_CLOSE_FORCE_TW_SKIP is required — shutdown
 * always passes flags=0. */
#define DPDK_NET_SHUT_RD    0
#define DPDK_NET_SHUT_WR    1
#define DPDK_NET_SHUT_RDWR  2
int32_t dpdk_net_shutdown(dpdk_net_engine_t*, dpdk_net_conn_t, int32_t how);
 ...
 * null `out`.
 *
 * Delta-math pattern: two successive snapshots with `wrapping_sub` per
 * bucket yield the increment over the interval, even across
 * wraparound, so long as no single bucket accumulated > 2^32 samples
 * between polls. */
typedef struct {
...
| 9293 §3.8.2 | URG mechanism (MUST-30/31) | **no** | URG-flagged inbound segments dropped; `tcp.rx_urgent_dropped` counter. Trading venues do not use URG; §1 non-goals. Deviation row `AD-A8-urg-dropped` in §6.4. |
...
| 8985 | RACK-TLP | yes | A5 implements RACK-TLP as the primary loss-detection path; 3-dup-ACK fast retrans is disabled (counter visibility only via `rx_dup_ack`). **A5.5 additions:** §6.3 `RACK_mark_losses_on_RTO` pass now runs at the top of `on_rto_fire` (Task 14 — was AD-17 in A5 review, closed). §7.2 `arm_tlp_pto` now called from the `Engine::send_bytes` TX path on every new-data send (Task 15 — was AD-18 in A5 review, closed). Per-connect tuning knobs (`tlp_pto_min_floor_us`, `tlp_pto_srtt_multiplier_x100`, `tlp_skip_flight_size_gate`, `tlp_max_consecutive_probes`, `tlp_skip_rtt_sample_gate`) deviate from strict §7.2 (PTO formula) and §7.3 step 2 (RTT-sample gate) when set — default values match RFC 8985 exactly. |
| 6298 §2.2 + §3 | SRTT from SYN handshake + Karn's rule | yes (A5.5) | SRTT is seeded from the SYN handshake round-trip on the first SYN's ACK per RFC 6298 §2.2 (first-RTT seeding). Karn's rule (§3) honored via `syn_retransmit_count == 0` guard — retransmitted SYNs produce no sample. `min_rtt_us` and `rto_us` are trustworthy from the moment the connection enters ESTABLISHED. Added A5.5 Task 13. |
...
| `AD-A8-urg-dropped` | RFC 9293 §3.8.2 + MUST-30/31 require an implementation to signal urgent data to the user (urgent-pointer semantics, SO_OOBINLINE-equivalent out-of-band queue, URG-echo in TX) | **always-on** drop behavior: URG-flagged inbound segments are dropped at the top of `established_rx` (`TxAction::None`, zero delivered bytes, no response segment) and `tcp.rx_urgent_dropped` is bumped. The stack emits no URG-bearing segment in any code path (TX flag construction never sets `TCP_URG`). | Stage 1 is a byte-stream raw-TCP API for trading REST/WS clients; the target exchange venues do not use URG and trading workloads have no out-of-band-data semantics. Full URG support (urgent-pointer buffer, SO_OOBINLINE-equivalent semantics, URG-echo in TX) is ~150 LoC of bookkeeping for zero trading value. Counter visibility (`tcp.rx_urgent_dropped`) gives the application a forensic signal on peer misbehavior. **Promotion gate:** any future phase requiring URG (not anticipated in Stages 1–5 per §1 non-goals) reopens this row. Citation: §1 non-goals + §6.3 RFC compliance matrix row `9293 | §3.8.2 URG | no`; exercised by `tools/tcpreq-runner/src/probes/urgent.rs` (passes with `ProbeStatus::Deviation("AD-A8-urg-dropped")`). |
| `AD-A8-dup-syn-in-syn-rcvd-mtcp-dispatch` | RFC 9293 §3.10.7.4 Fourth (SYN in synchronized state) prescribes "return this connection to the LISTEN state" for passive-OPEN conns on a received SYN, regardless of SEG.SEQ | **feature=test-server, dispatched by SEG.SEQ:** (1) SEG.SEQ == IRS (benign peer-SYN-retransmit): retransmit the stored SYN-ACK via `emit_syn_ack_for_passive` (reusing the existing T11 SynRetrans wheel entry; no re-arm). (2) SEG.SEQ != IRS (in-window SYN with advanced seq): `TxAction::Rst` + `new_state: Closed` + `clear_listen_slot_on_close` (T12 cleanup path). Neither branch transitions to LISTEN. | Two branches, two rationales. **Benign case**: the mTCP AD-4 pattern (`third_party/mtcp/mtcp/src/tcp_in.c:1308-1311`) reads §3.10.7.4 Fourth's `may` latitude + §3.8.1's "control segments retransmitted until ACKed" as licensing SYN-ACK retransmit rather than listener re-entry. Retransmit is the smaller behavior change for the most common trigger (peer's final ACK was merely delayed), avoids tearing down the half-open TCB, and preserves RTO/budget state for the ongoing handshake. **Non-benign case**: SEG.SEQ advancing mid-handshake is protocol-level confusion (advanced-ISS, dup 4-tuple from a stale incarnation, or malicious). RST-on-RFC-violating-SYN is safer for the single-slot test-server (capacity-1 listen per `AD-1` mTCP divergence) — return-to-LISTEN would leave the slot open to repeated confusion, RST signals the peer to resynchronize. Project rule spec §6 line 365 ("never transition to LISTEN in production") applies to the production build by construction; the test-server path is compile-gated (`#[cfg(feature = "test-server")]`) so the rule remains structurally intact. **Promotion gate:** if AD-1 retires (multi-slot listen backlog), revisit the non-benign branch — RST-then-close may become inappropriate when multiple concurrent handshakes share a listener, and RFC's "return to LISTEN" becomes more natural per-conn. Citation: A8 design spec §4.4; A8 T14 commit `30fde2a`; regression test `crates/dpdk-net-core/tests/ad_a7_dup_syn_retrans_synack.rs` (both branches covered); cross-referenced A8 mTCP review `AD-4-strict-new` and A8 RFC review S-1. |
| `AD-A8.5-shutdown-no-half-close` | POSIX `shutdown(2)` supports half-close via `SHUT_RD` / `SHUT_WR`, allowing the caller to close one direction while keeping the other open | `dpdk_net_shutdown(h, DPDK_NET_SHUT_RD)` and `dpdk_net_shutdown(h, DPDK_NET_SHUT_WR)` both return `-EOPNOTSUPP`. Only `DPDK_NET_SHUT_RDWR` performs the close (dispatching to `dpdk_net_close(h, 0)`). | Stage 1 byte-stream API has no half-close consumer. Half-close carries TCB edge cases (RX-side deliver-after-SHUT_RD semantics; TX-side retransmit-on-half-closed-write timing; RST timing interactions) that Stage 1 doesn't need — implementing untested half-close paths is a net regression risk. Full-close covers every current use (REST/WS clients terminate the full connection). mTCP precedent: `mtcp/src/include/mtcp_api.h` exposes no `mtcp_shutdown()` API at all — a stronger version of the same decision. **Promotion gate:** HTTP/1.1 pipelining (Stage 3) or WebSocket close-frame handling (Stage 5) — both need half-close semantics — reopens this row. Citation: §4 API surface + A8.5 design spec §4.3; exercised by `crates/dpdk-net/tests/api_shutdown.rs` (PASS-with-EOPNOTSUPP for SHUT_RD + SHUT_WR). |
| `AD-A8.5-rst-urg-precedence` | RFC 9293 §3.5.3 + §3.10.7.4: "RST is processed independently of other flags" — a peer-sent `RST\|URG` on an ESTABLISHED connection must abort the connection per §3.5.3 line 1518 | Consequence of pre-existing `AD-A8-urg-dropped`: the URG-drop check at `tcp_input.rs:730` runs *before* the RST check at `:744`, so `RST\|URG` segments are dropped at the top of `established_rx` (`TxAction::None`, `urgent_dropped=true`, `closed=false`, `new_state=None`). The engine still bumps `tcp.rx_rst` at `engine.rs:3617` on flag presence alone (before per-state dispatch), plus `tcp.rx_urgent_dropped`. The connection remains ESTABLISHED. Plain RST (without URG) correctly closes per §3.5.3. | Direct consequence of the `AD-A8-urg-dropped` drop-on-URG ordering; fixing it would require either (a) re-ordering RST-before-URG in `handle_established` (risks subtle interactions with `AD-A8-urg-dropped`'s always-on URG-drop invariant and its counter-coverage unit tests) or (b) implementing a full URG path (scope-rejected by §1 non-goals). Trading venues do not send `RST\|URG` — the combination is a pathological peer behavior, not an expected production path. Counter visibility via `tcp.rx_rst` + `tcp.rx_urgent_dropped` gives forensic signal if a peer ever uses this combination. **Promotion gate:** any phase that reopens `AD-A8-urg-dropped` (URG scope-in) automatically retires this row — the URG-precedence ordering disappears when URG segments are no longer dropped. Citation: A8.5 RFC compliance review S-1 resolution; exercised by `tools/tcpreq-runner/src/probes/rst_ack.rs` scenario B (asserts `+1 tcp.rx_rst` AND `+1 tcp.rx_urgent_dropped` on `RST\|URG` — pinning the deviation rather than claiming full independence). |
| `AD-A8.5-tx-wscale-position` | RFC 9293 §3.2 is receiver-order-agnostic (any TX ordering is RFC-compliant); Linux kernel `net/ipv4/tcp_output.c::tcp_options_write` emits WSCALE last in `<MSS, SACKP, TS, NOP+WSCALE>` order | We emit `<MSS, NOP+WSCALE, SACKP, TS, [NOPs+SACK-blocks]>` — WSCALE second (right after MSS), SACKP and TS after WSCALE. Matches the shivansh and ligurio packetdrill corpus SYN-ACK hand-crafted `tcp.opt(...)` expectations. Does NOT match modern Linux (WSCALE last), Google packetdrill upstream (WSCALE last), or mTCP (WSCALE last). | Corpus-compatibility choice: shivansh and ligurio corpora were our Layer-B unlock path for A8.5 T5. Byte-exact wire shape is the contract those corpora assert against; changing to WSCALE-last would lock out both corpora while unlocking at most a handful of google-upstream scripts (which remain blocked on deeper engine gaps per A8.5 T6 post-mortem). Trading workloads never depend on a specific TX option ordering. **Promotion gate:** if Stage 2+ selects Google upstream corpus as the primary Layer-B target (currently ligurio + shivansh hold that role), flip the order to WSCALE-last and retire this row. Citation: A8.5 mTCP comparison review AD-1; T5 commit `955f34a` (reorder) + `8ec5d2d` (return `Some(i)` drift fix); documented in `crates/dpdk-net-core/src/tcp_options.rs` module comment. |
