...
pub(crate) struct LlqVerdict {
    ...
    /// setups bypass dpdk_net_eal_init entirely.
    pub captured: bool,
}
...
pub(crate) fn record_eal_init_log_verdict(captured_log: &str) {
    ...
    if captured_log.trim().is_empty() {
        eprintln!(
            "dpdk_net: LLQ log capture returned empty buffer around rte_eal_init; \
...
///     bypass `dpdk_net_eal_init`). Emits a warning to stderr but does
...
pub(crate) fn verify_llq_activation_from_global(
    ...
    let Some(verdict) = EAL_INIT_VERDICT.get() else {
        ...
        // happens in unit-test setups that bypass dpdk_net_eal_init, or
        ...
        eprintln!(
            "dpdk_net: port {} driver=net_ena but no EAL-init LLQ capture recorded; \
             skipping LLQ verification. Ensure dpdk_net_eal_init runs on real ENA hosts.",
            port_id
        );
        return Ok(());
    };
    ...
    if verdict.has_failure || !verdict.has_activation {
        ...
        eprintln!(
            "dpdk_net: port {} driver=net_ena but LLQ did not activate at EAL init \
...
pub(crate) fn verify_llq_activation(
    ...
    if verdict.has_failure || !verdict.has_activation {
        counters
            ...
            .fetch_add(1, Ordering::Relaxed);
        eprintln!(
            "dpdk_net: port {} driver=net_ena but LLQ did not activate at bring-up \
             (has_failure={}, has_activation={}). Failing hard per spec §5.\n\
             --- captured PMD log ---\n{}\n--- end log ---",
