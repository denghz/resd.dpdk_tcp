#!/usr/bin/env python3
"""T53 fast-iter result analyzer — dpdk_net vs fstack vs linux comparison.
...
dpdk_b = load_csv(f"{T53}/fast-burst-dpdk.csv", "dpdk_net")
fstk_b = load_csv(f"{T53}/fast-burst-fstack.csv", "fstack")
...
print("  linux:    same (wire rate — drain fix applied in T53)")
print("  fstack:   ff_write accepted bytes (buffer-fill at low C)")
print("=" * 70)

dpdk_m = load_csv(f"{T53}/fast-maxtp-dpdk-linux.csv", "dpdk_net")
lnx_m  = load_csv(f"{T53}/fast-maxtp-dpdk-linux.csv", "linux")
fstk_m = load_csv(f"{T53}/fast-maxtp-fstack.csv", "fstack")
...
print(f"\nLinux drain-fix verification (T53 vs T51):")
