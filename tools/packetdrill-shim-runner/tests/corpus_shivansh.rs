...
//! A8 T16: shivansh-tcp-ip-regression corpus runner.
//!
//! Mirrors tests/corpus_ligurio.rs. Classifier TOML lives at
//! tools/packetdrill-shim/classify/shivansh.toml and is read at
//! test-runtime (not `include_str!`, because `include_str!` requires a
//! literal path and can't be parameterized across corpora).
//!
//! Runs every classifier-runnable script through the shim binary and
//! asserts each exits 0. Also asserts SHIVANSH_RUNNABLE_COUNT /
//! SHIVANSH_SKIP_* match the pinned constants in counts.rs and every
//! skipped script has an entry in SKIPPED.md.
...
const CORPUS_ROOT: &str = "../../third_party/shivansh-tcp-ip-regression";
const CLASSIFY_TOML: &str =
    "../../tools/packetdrill-shim/classify/shivansh.toml";

#[test]
fn shivansh_runnable_subset_passes() {
    ...
    // Pinned-count check (T16 pins these based on the A8 T15 S2 shim).
    assert_eq!(runnable.len(),     counts::SHIVANSH_RUNNABLE_COUNT,
        "runnable count drift — update counts::SHIVANSH_RUNNABLE_COUNT");
    assert_eq!(skip_untrans.len(), counts::SHIVANSH_SKIP_UNTRANSLATABLE);
    assert_eq!(skip_oos.len(),     counts::SHIVANSH_SKIP_OUT_OF_SCOPE);
}
