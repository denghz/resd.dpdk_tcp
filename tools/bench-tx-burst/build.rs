//! bench-tx-burst build script — links libfstack.a when the `fstack`
//! cargo feature is enabled. No-op otherwise so default workspace
//! builds don't require F-Stack on the dev host.
//!
//! Phase 5 of the 2026-05-09 bench-suite overhaul split bench-vs-mtcp
//! into bench-tx-burst + bench-tx-maxtp; this build.rs is verbatim
//! from the legacy bench-vs-mtcp/build.rs (and matches bench-rtt's
//! build.rs because both consume libfstack.a). The fstack-ffi
//! consolidation tracked in the plan would let all three share one
//! build script + ffi crate.
//!
