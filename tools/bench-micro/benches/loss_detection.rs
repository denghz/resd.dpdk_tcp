...
//!   with new SACK data).
//! * `bench_sack_scoreboard_insert_merge` — scoreboard pre-loaded with
//!   3 disjoint blocks; insert a 4th non-overlapping block, filling
//!   the array to its 4-entry capacity (`MAX_SACK_SCOREBOARD_ENTRIES`,
//!   tcp_sack.rs:11). Measures the merge-search loop crossing the full
//!   array plus the append. Call site: same as above.
...
//! * `bench_retrans_push_after_tx` — empty `SendRetrans` + one
//!   `push_back` of a `RetransEntry` literal. Isolates the deque-push
//!   cost from T1's bundled per-segment build path. Call site:
//!   engine.rs send loop (every TX'd segment).
...
/// non-overlapping block. The merge-search loop iterates 3 times, the
/// append fires (count goes from 3 to 4 — exactly
/// MAX_SACK_SCOREBOARD_ENTRIES, no eviction yet). This is the
/// "scoreboard full but not overflowing" shape that a typical
/// 3-reorder session reaches steady-state at.
fn bench_sack_scoreboard_insert_merge(c: &mut Criterion) {
    c.bench_function("bench_sack_scoreboard_insert_merge", |b| {
        b.iter_batched_ref(
...
/// true. Per-byte cost is the same as a real RACK scan iterating one
/// segment's range and probing the scoreboard.
...
/// Empty `SendRetrans` + one `push_after_tx`. Isolates the deque-push
/// cost from T1's bundled per-segment build path. T1 measures
/// segment-build + checksum + mbuf-copy + push together; this target
/// measures push alone.
///
/// Note: criterion's per-iter `iter_batched_ref` setup closure builds
/// a fresh `SendRetrans` each iteration. The VecDeque backing it is
/// freshly allocated per-iter — the `with_capacity` pre-size at
/// tcp_retrans.rs:65 is NOT used here, so this bench measures the
/// first-push path on a default-empty VecDeque (which calls
/// `reserve(1)` internally on first insert). Production at the call
/// site has a pre-sized deque from the connection-construction
/// `with_capacity`; this target is therefore a SLIGHT overestimate
/// of the steady-state per-push cost.
...
criterion_group! {
    ...
    targets =
        ...
        bench_sack_scoreboard_insert_merge,
